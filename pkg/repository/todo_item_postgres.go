...
func (r *TodoItemPostgres) GetAll(userId, listId int) ([]todo.TodoItem, error) {
	...
	query := fmt.Sprintf("SELECT ti.title, ti.description, ti.done FROM %s ti INNER JOIN %s li on li.item_id = ti.id INNER JOIN %s ul on ul.list_id = li.list_id WHERE li.list_id = $1 AND ul.user_id = $2",
...
func (r *TodoItemPostgres) Delete(userId, itemId int) error {
	logrus.Infof("==========")
	logrus.Infof(strconv.Itoa(userId))
	logrus.Infof(strconv.Itoa(itemId))
